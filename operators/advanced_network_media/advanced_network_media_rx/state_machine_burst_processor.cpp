...
StateMachineBurstProcessor::StateMachineBurstProcessor(
    std::shared_ptr<StateMachinePacketsToFramesConverter> converter)
    : converter_(converter) {
  if (!converter_) {
    throw std::invalid_argument("StateMachinePacketsToFramesConverter cannot be null");
  }
}

void StateMachineBurstProcessor::process_burst(BurstParams* burst, bool hds_enabled) {
...
void StateMachineBurstProcessor::configure_converter_from_burst(BurstParams* burst) {
...
void StateMachineBurstProcessor::process_packets_in_burst(BurstParams* burst, bool hds_enabled) {
  ...
  for (size_t i = 0; i < burst->hdr.hdr.num_pkts; ++i) {
    ...
    if (payload) {
      PACKET_TRACE_LOG("About to process packet {}/{}: seq={}, m_bit={}, size={}, payload_ptr={}",
                       i + 1, burst->hdr.hdr.num_pkts,
                       rtp_params.sequence_number, rtp_params.m_bit, rtp_params.payload_size,
...
uint8_t* StateMachineBurstProcessor::extract_packet_data(BurstParams* burst, 
                                                      size_t packet_index, 
                                                      bool hds_enabled,
                                                      RtpParams& rtp_params) {
  if (packet_index >= burst->hdr.hdr.num_pkts) {
    HOLOSCAN_LOG_ERROR("Packet index {} out of range (max: {})", 
                       packet_index, burst->hdr.hdr.num_pkts);
    return nullptr;
  }
  ...
  if (hds_enabled) {
    ...
    if (!header_ptr || !payload_ptr) {
      HOLOSCAN_LOG_ERROR("Null pointer in HDS packet {}: header={}, payload={}",
                         packet_index, static_cast<void*>(header_ptr), 
    ...
    PACKET_TRACE_LOG("HDS packet {}: header_ptr={}, payload_ptr={}, seq={}", 
                     packet_index, static_cast<void*>(header_ptr), 
                     static_cast<void*>(payload_ptr), rtp_params.sequence_number);
    ...
    PACKET_TRACE_LOG("Standard packet {}: packet_ptr={}, payload_ptr={}, seq={}", 
                     packet_index, static_cast<void*>(packet_ptr), 
                     static_cast<void*>(payload_ptr), rtp_params.sequence_number);
