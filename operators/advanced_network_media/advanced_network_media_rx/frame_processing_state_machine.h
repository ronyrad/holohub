...
#ifndef OPERATORS_ADVANCED_NETWORK_MEDIA_RX_FRAME_PROCESSING_STATE_MACHINE_H_
#define OPERATORS_ADVANCED_NETWORK_MEDIA_RX_FRAME_PROCESSING_STATE_MACHINE_H_
...
class IStateMachineAwareStrategy;
...
/**
 * @brief Strategy interface that works with the state machine
 */
class IStateMachineAwareStrategy {
public:
  virtual ~IStateMachineAwareStrategy() = default;
  
  /**
   * @brief Process packet with state machine coordination
   * @param state_machine Reference to state machine
   * @param payload Packet payload data
   * @param payload_size Size of payload
   * @return Event generated by processing
   */
  virtual StateEvent process_packet(class FrameProcessingStateMachine& state_machine,
                                   uint8_t* payload, 
                                   size_t payload_size) = 0;
  
  /**
   * @brief Execute pending copy operations
   * @param state_machine Reference to state machine for context updates
   * @return Event generated by copy execution
   */
  virtual StateEvent execute_pending_copy(class FrameProcessingStateMachine& state_machine) = 0;
 ...
 * error handling.
 */
class FrameProcessingStateMachine {
public:
  /**
   * @brief Constructor
   * @param frame_provider Provider for frame allocation
   */
  explicit FrameProcessingStateMachine(std::shared_ptr<IFrameProvider> frame_provider);
  
  /**
   * @brief Process state machine event
   * @param event Event to process
   * @param rtp_params RTP packet parameters (optional, can be nullptr)
   * @param payload Packet payload (optional, can be nullptr)
  ...
  /**
   * @brief Get current state machine context (read-only)
   * @return Current context
   */
  const StateMachineContext& get_context() const { return context_; }
   ...
   */
  void set_strategy(std::shared_ptr<IStateMachineAwareStrategy> strategy);
  
  /**
   * @brief Get currently set strategy
   * @return Current strategy or nullptr
   */
  std::shared_ptr<IStateMachineAwareStrategy> get_strategy() const { return strategy_; }
...
private:
  ...
  std::shared_ptr<IStateMachineAwareStrategy> strategy_;
...
/**
 * @brief Utility functions for state machine operations
 */
...
#endif  // OPERATORS_ADVANCED_NETWORK_MEDIA_RX_FRAME_PROCESSING_STATE_MACHINE_H_ 