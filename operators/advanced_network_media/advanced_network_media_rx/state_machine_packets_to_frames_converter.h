...
#ifndef OPERATORS_ADVANCED_NETWORK_MEDIA_RX_STATE_MACHINE_PACKETS_TO_FRAMES_CONVERTER_H_
#define OPERATORS_ADVANCED_NETWORK_MEDIA_RX_STATE_MACHINE_PACKETS_TO_FRAMES_CONVERTER_H_
 ...
 */
class StateMachinePacketsToFramesConverter {
public:
  /**
   * @brief Constructor
   ...
   */
  StateMachinePacketsToFramesConverter(std::shared_ptr<IFrameProvider> frame_provider,
  ...
  struct Statistics {
    ...
    CopyStrategy current_strategy = CopyStrategy::UNKNOWN;
    FrameState current_frame_state = FrameState::IDLE;
...
private:
   ...
   * @param payload Packet payload
   */
  void execute_actions(const StateTransitionResult& result, 
   ...
   */
  void setup_strategy(std::unique_ptr<IStateMachineAwareStrategy> strategy);
...
private:
  // Core components
  std::unique_ptr<FrameProcessingStateMachine> state_machine_;
  std::unique_ptr<StrategyDetector> strategy_detector_;
  std::unique_ptr<IStateMachineAwareStrategy> current_strategy_;
...
public:
   ...
   */
  static ConverterConfiguration create_from_burst_config(
...
#endif  // OPERATORS_ADVANCED_NETWORK_MEDIA_RX_STATE_MACHINE_PACKETS_TO_FRAMES_CONVERTER_H_ 