...
// ========================================================================================
// StateMachinePacketsToFramesConverter Implementation
// ========================================================================================

StateMachinePacketsToFramesConverter::StateMachinePacketsToFramesConverter(
  ...
  state_machine_ = std::make_unique<FrameProcessingStateMachine>(frame_provider);
  ...
  HOLOSCAN_LOG_INFO("StateMachinePacketsToFramesConverter initialized: strategy_detection={}, force_contiguous={}",
                    config_.enable_strategy_detection, config_.force_contiguous_strategy);
}

void StateMachinePacketsToFramesConverter::set_completion_handler(
    std::shared_ptr<IFrameCompletionHandler> handler) {
  completion_handler_ = handler;
}

void StateMachinePacketsToFramesConverter::configure_burst_parameters(size_t header_stride_size, 
  ...
  if (strategy_detector_) {
    strategy_detector_->configure_burst_parameters(header_stride_size, payload_stride_size, hds_enabled);
  }
...
void StateMachinePacketsToFramesConverter::configure_memory_types(
    nvidia::gxf::MemoryStorageType source_type,
  ...
  HOLOSCAN_LOG_DEBUG("Memory types configured: source={}, destination={}",
                     static_cast<int>(source_type), static_cast<int>(destination_type));
  ...
  if (current_strategy_ && !strategy_detection_active_) {
    ...
    if (strategy_type == CopyStrategy::CONTIGUOUS) {
      current_strategy_ = StrategyFactory::create_contiguous_strategy(source_type, destination_type);
    }
...
void StateMachinePacketsToFramesConverter::process_incoming_packet(const RtpParams& rtp_params, 
                                                                uint8_t* payload) {
  try {
    // Update statistics
    ...
    PACKET_TRACE_LOG("Packet processed successfully: seq={}, event={}, new_state={}",
                       rtp_params.sequence_number, static_cast<int>(event), 
...
void StateMachinePacketsToFramesConverter::force_strategy_redetection() {
  if (strategy_detector_) {
...
void StateMachinePacketsToFramesConverter::reset() {
  state_machine_->reset();
  
  if (strategy_detector_) {
    strategy_detector_->reset();
    strategy_detection_active_ = config_.enable_strategy_detection && !config_.force_contiguous_strategy;
  }
  ...
  statistics_.current_strategy = CopyStrategy::UNKNOWN;
  statistics_.current_frame_state = FrameState::IDLE;
  statistics_.last_error.clear();
  
  HOLOSCAN_LOG_INFO("Converter reset to initial state");
}

StateMachinePacketsToFramesConverter::Statistics StateMachinePacketsToFramesConverter::get_statistics() const {
  // Update current state information
  const auto& context = state_machine_->get_context();
  statistics_.current_frame_state = context.frame_state;
  
  if (current_strategy_) {
    statistics_.current_strategy = current_strategy_->get_type();
  }
  
  return statistics_;
}

bool StateMachinePacketsToFramesConverter::has_pending_operations() const {
  const auto& context = state_machine_->get_context();
  return context.has_pending_copy || (current_strategy_ && current_strategy_->has_pending_operations());
}

std::shared_ptr<FrameBufferBase> StateMachinePacketsToFramesConverter::get_current_frame() const {
  return state_machine_->get_current_frame();
}

size_t StateMachinePacketsToFramesConverter::get_frame_position() const {
  return state_machine_->get_frame_position();
}

StateEvent StateMachinePacketsToFramesConverter::determine_event(const RtpParams& rtp_params, 
                                                              uint8_t* payload) {
  ...
  if (strategy_detection_active_ && strategy_detector_) {
    if (strategy_detector_->collect_packet(rtp_params, payload, rtp_params.payload_size)) {
      ...
      auto detected_strategy = strategy_detector_->detect_strategy(
          config_.source_memory_type, config_.destination_memory_type);
...
void StateMachinePacketsToFramesConverter::execute_actions(const StateTransitionResult& result, 
                                                        const RtpParams& rtp_params, 
                                                        uint8_t* payload) {
  PACKET_TRACE_LOG("execute_actions: should_emit_frame={}, should_complete_frame={}, new_state={}", 
                   result.should_emit_frame, result.should_complete_frame, static_cast<int>(result.new_frame_state));
  // Strategy processing
  if ((result.new_frame_state == FrameState::RECEIVING_PACKETS ||
    ...
    StateEvent strategy_result = current_strategy_->process_packet(*state_machine_, payload, rtp_params.payload_size);
...
bool StateMachinePacketsToFramesConverter::handle_strategy_detection(const RtpParams& rtp_params, 
                                                                  uint8_t* payload) {
  if (!strategy_detection_active_ || !strategy_detector_) {
...
void StateMachinePacketsToFramesConverter::setup_strategy(std::unique_ptr<IStateMachineAwareStrategy> strategy) {
  ...
  // but since IStateMachineAwareStrategy is different from IPacketCopyStrategy, we manage it here
  
  if (current_strategy_) {
    HOLOSCAN_LOG_INFO("Strategy setup completed: {}", 
                      current_strategy_->get_type() == CopyStrategy::CONTIGUOUS ? "CONTIGUOUS" : "STRIDED");
  }
}

bool StateMachinePacketsToFramesConverter::validate_packet_integrity(const RtpParams& rtp_params) {
  auto frame = state_machine_->get_current_frame();
  if (!frame) {
    return false;
  }
...
void StateMachinePacketsToFramesConverter::handle_frame_completion() {
  ...
  if (result.should_emit_frame) {
    ...
    // Signal frame emission complete to transition FRAME_READY -> IDLE
    PACKET_TRACE_LOG("Sending FRAME_COMPLETED event to transition FRAME_READY -> IDLE from handle_frame_completion");
    auto emission_result = state_machine_->process_event(StateEvent::FRAME_COMPLETED);
    if (!emission_result.success) {
      HOLOSCAN_LOG_ERROR("Failed to complete frame emission in handle_frame_completion: {}", emission_result.error_message);
    } else {
...
void StateMachinePacketsToFramesConverter::handle_error_recovery(const std::string& error_message) {
...
void StateMachinePacketsToFramesConverter::update_statistics(StateEvent event) {
  switch (event) {
...
ConverterConfiguration ConverterConfigurationHelper::create_from_burst_config(
  ...
  config.source_memory_type = payload_on_cpu 
      ? nvidia::gxf::MemoryStorageType::kHost 
      : nvidia::gxf::MemoryStorageType::kDevice;
      
  config.destination_memory_type = frames_on_host 
      ? nvidia::gxf::MemoryStorageType::kHost 
      : nvidia::gxf::MemoryStorageType::kDevice;
...
bool ConverterConfigurationHelper::validate_configuration(const ConverterConfiguration& config) {
  ...
  if (config.enable_strategy_detection && config.force_contiguous_strategy) {
    HOLOSCAN_LOG_ERROR("Configuration error: Cannot enable strategy detection and force contiguous strategy simultaneously");
    return false;
  }
  ...
  if (config.enable_strategy_detection) {
    if (config.header_stride_size == 0 && config.payload_stride_size == 0) {
      HOLOSCAN_LOG_WARN("Zero stride sizes with strategy detection enabled may affect detection accuracy");
    }
  }
