...
/**
 * @brief Consumer class for handling Rivermax chunks and providing ANO bursts.
 *
 * The RivermaxChunkConsumerAno class acts as an adapter that consumes Rivermax chunks
...
class RivermaxChunkConsumerAno : public IReceiveDataConsumer {
 public:
  ...
  explicit RivermaxChunkConsumerAno(std::shared_ptr<RxPacketProcessor> packet_processor)
      : m_packet_processor(std::move(packet_processor)) {
    m_packet_info_array = std::make_unique<ReceivePacketInfo[]>(RivermaxBurst::MAX_PKT_IN_BURST);
  }
 ...
 protected:
  ...
  std::unique_ptr<ReceivePacketInfo[]> m_packet_info_array;
};
...
inline ReturnStatus RivermaxChunkConsumerAno::consume_chunk(const ReceiveChunk& chunk,
  ...
  for (size_t i = 0; i < chunk_size; ++i) { m_packet_info_array[i] = chunk.get_packet_info(i); }
  ...
  auto process_status = m_packet_processor->process_packets(params, consumed_packets);
