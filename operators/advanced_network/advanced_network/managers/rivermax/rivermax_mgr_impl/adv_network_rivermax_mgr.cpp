...
/**
 * A map of Ano log level to Rivermax log level.
 */
const std::unordered_map<LogLevel::Level, RivermaxLogLevel::Level>
    RivermaxLogLevel::ano_to_rivermax_log_level_map = {
...
class RivermaxMgr::RivermaxMgrImpl {
 ...
 private:
  ...
  void initialize_rx_service(uint32_t service_id,
...
void RivermaxMgr::RivermaxMgrImpl::initialize_rx_service(
  ...
  if (config_type == QueueConfigType::IPOReceiver) {
    ...
    send_packet_ext_info = ipo_receiver_builder->send_packet_ext_info;
    ...
    send_packet_ext_info = rtp_receiver_builder->send_packet_ext_info;
    auto init_status = rx_service->initialize();
    if (init_status != ReturnStatus::obj_init_success) {
      HOLOSCAN_LOG_ERROR("Failed to initialize RX service, status: {}", (int)init_status);
      return;
    ...
    max_chunk_size = rtp_receiver_builder->max_chunk_size;
  } else {
  ...
  rx_burst_managers[service_id] = std::make_shared<RxBurstsManager>(
      send_packet_ext_info, port_id, queue_id, max_chunk_size, gpu_id, rx_bursts_out_queue);
  ...
  rx_services[service_id]->set_receive_data_consumer(0, std::move(rivermax_chunk_consumer));
}
...
Status RivermaxMgr::RivermaxMgrImpl::get_rx_burst(BurstParams** burst) {
...
void RivermaxMgr::RivermaxMgrImpl::print_stats() {
  std::stringstream ss;
  RxStatsPrinter::print_total_stats(ss, rx_services);
...
Status RivermaxMgr::get_rx_burst(BurstParams** burst) {
