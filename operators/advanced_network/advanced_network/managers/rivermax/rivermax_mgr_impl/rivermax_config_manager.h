...
class TypedConfigBuilderHolder : public ConfigBuilderHolder {
 private:
  QueueConfigType m_type;
  std::shared_ptr<ConfigBuilderType> m_config_builder;
 ...
 public:
  ...
  TypedConfigBuilderHolder(QueueConfigType type, std::shared_ptr<ConfigBuilderType> config_builder)
      : m_type(type), m_config_builder(std::move(config_builder)) {}
   ...
   * @return: The configuration type enum value.
   */
  QueueConfigType get_type() const override { return m_type; }
   ...
   */
  std::shared_ptr<ConfigBuilderType> get_config_builder() const { return m_config_builder; }
};
...
class ConfigBuilderContainer {
 private:
  std::map<uint32_t, std::shared_ptr<ConfigBuilderHolder>> m_holders;
 ...
 public:
   ...
   */
  ConstIterator begin() const { return m_holders.begin(); }
  ConstIterator end() const { return m_holders.end(); }
  ConstIterator cbegin() const { return m_holders.cbegin(); }
  ConstIterator cend() const { return m_holders.cend(); }
  ...
  void add_config_builder(uint32_t config_num, QueueConfigType type,
    ...
    m_holders[config_num] = std::make_shared<TypedConfigBuilderHolder<ConfigBuilderType>>(
  ...
  std::shared_ptr<ConfigBuilderHolder> get_holder(uint32_t config_num) const {
    auto it = m_holders.find(config_num);
    if (it != m_holders.end()) { return it->second; }
    return nullptr;
  }
  ...
  bool has_config(uint32_t config_num) const {
    return m_holders.find(config_num) != m_holders.end();
  }
  ...
  std::vector<uint32_t> get_config_nums() const {
    ...
    keys.reserve(m_holders.size());
    for (const auto& pair : m_holders) { keys.push_back(pair.first); }
    return keys;
  }
   ...
   */
  size_t size() const { return m_holders.size(); }

  /**
   * @brief: Clears all configurations from the container.
   */
  void clear() { m_holders.clear(); }
};
...
class RivermaxConfigContainer {
 ...
 private:
  ...
  /**
   * @brief Sets the Rivermax log level based on the provided Ano log level.
   *
   * This function converts the provided Ano log level to the corresponding Rivermax log level
   * and sets it as the current log level for Rivermax.
   *
   * @param level The Ano log level to be converted and set.
   */
  void set_rivermax_log_level(LogLevel::Level level) {
    rivermax_log_level_ = RivermaxLogLevel::from_ano_log_level(level);
  }
