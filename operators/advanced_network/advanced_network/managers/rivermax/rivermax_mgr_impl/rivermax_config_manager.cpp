...
bool RxConfigManager::append_ipo_receiver_candidate_for_rx_queue(
  ...
  settings_builder->send_packet_ext_info = rivermax_rx_config.send_packet_ext_info;
...
bool RivermaxConfigParser::parse_common_tx_settings(
  ...
  rivermax_tx_config.local_ip = tx_settings["local_ip"].as<std::string>("");
  rivermax_tx_config.destination_ip = tx_settings["destination_ip"].as<std::string>("");
...
bool ConfigManagerUtilities::validate_memory_regions_config(
  ...
  uint16_t num_of_mrs = memory_regions.size();
  try {
    if (num_of_mrs == 1) {
