...
class RivermaxLogLevel {
 public:
  ...
  /**
   * @brief Converts an Ano log level to an Rivermax log level.
   *
   * @param ano_level The Ano log level to convert.
   * @return The corresponding Rivermax log level.
   */
  static Level from_ano_log_level(LogLevel::Level ano_level) {
    auto it = ano_to_rivermax_log_level_map.find(ano_level);
    if (it != ano_to_rivermax_log_level_map.end()) { return it->second; }
    return OFF;
  }
 ...
 private:
   ...
   */
  static const std::unordered_map<LogLevel::Level, Level> ano_to_rivermax_log_level_map;
};
