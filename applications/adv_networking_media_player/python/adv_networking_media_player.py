...
HARDCODED_SIZE = [1920, 1080]
HARDCODED_ROI = [0, 0] + HARDCODED_SIZE
...
class App(Application):
    def compose(self):
        ...
        # Create CUDA stream pool for CUDA operations
        ...
        try:
            ...
            adv_net_media_rx = adv_network_media_rx.AdvNetworkMediaRxOp(
                ...
                frame_width=rx_config.get("frame_width", 1920),
                frame_height=rx_config.get("frame_height", 1080),
                bit_depth=rx_config.get("bit_depth", 8),
                video_format=rx_config.get("video_format", "RGB888"),
                hds=rx_config.get("hds", True),
                output_format=rx_config.get("output_format", "video_buffer"),
                memory_location=rx_config.get("memory_location", "device"),
        ...
        try:
            media_config = self.kwargs("media_player_config")
        except Exception as e:
            logger.error(f"Failed to get media_player_config: {e}")
            sys.exit(1)

        if not media_config.get("visualize", False):
            logger.error(
                "Visualization must be enabled. File writing is not supported in this Python version."
            )
            sys.exit(1)

        # Set up visualization pipeline
        try:
            # Get input format and convert it
            input_format = self._get_video_input_format(media_config.get("input_format", "rgb888"))
            ...
            format_converter = OrsiFormatConverterOp(
                ...
                output_img_size=HARDCODED_SIZE,
                src_roi_rect=HARDCODED_ROI,
            )
            ...
            # Set up the pipeline
            self.add_flow(adv_net_media_rx, format_converter)
    ...
    def _get_video_input_format(self, format_str):
        """
        Convert input format string to standardized format.
        ...
        if format_lower in ["rgb888", "rgb"]:
            return "rgb888"

        # YCbCr 4:2:0 / YUV 4:2:0 formats
        if format_lower in ["ycbcr-4:2:0", "yuv420", "yuv-420", "yuv-4:2:0", "ycbcr420"]:
            return "yuv420"

        if format_lower in ["nv12", "nv12_er", "nv12_709", "nv12_709_er"]:
            return "nv12"

        logger.error(f"Unsupported video sampling format: {format_str}")
        raise RuntimeError(f"Unsupported video sampling format: {format_str}")
