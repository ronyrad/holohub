...
class FramesWriterOp : public Operator {
 public:
  ...
  void initialize() override {
    ...
    if (!file_stream_) {
      HOLOSCAN_LOG_ERROR("Failed to open output file: {}. Check permissions and disk space.", file_path);
      ...
      if (path.has_parent_path()) {
        ...
        if (!ec) {
          ...
          HOLOSCAN_LOG_ERROR("Parent directory permissions: {}", 
  ...
  void compute(InputContext& op_input, [[maybe_unused]] OutputContext&,
    ...
    if (frames_recorded_ > num_of_frames_to_record_.get()) {
      return;
    }
 ...
 private:
  ...
  void process_video_buffer(nvidia::gxf::Handle<nvidia::gxf::VideoBuffer> video_buffer) {
    ...
    HOLOSCAN_LOG_TRACE("Processing VideoBuffer: size={}, storage_type={}", 
                       buffer_size, static_cast<int>(storage_type));
  ...
  void process_gxf_tensor(nvidia::gxf::Handle<nvidia::gxf::Tensor> tensor) {
    ...
    HOLOSCAN_LOG_TRACE("Processing Tensor: size={}, storage_type={}", 
                       tensor_size, static_cast<int>(storage_type));
  ...
  void write_data_to_file(void* data_ptr, size_t data_size, 
    ...
    if (!data_ptr || data_size == 0) {
      HOLOSCAN_LOG_ERROR("Invalid data pointer or size: ptr={}, size={}", 
                         static_cast<void*>(data_ptr), data_size);
      return;
    }
    ...
    if (host_buffer_.size() < data_size) {
      HOLOSCAN_LOG_TRACE("Resizing host buffer from {} to {} bytes", 
                         host_buffer_.size(), data_size);
    ...
    if (!file_stream_.good()) {
      HOLOSCAN_LOG_ERROR("Failed to write data to file - stream state: fail={}, bad={}, eof={}", 
                         file_stream_.fail(), file_stream_.bad(), file_stream_.eof());
    ...
    HOLOSCAN_LOG_TRACE("Successfully wrote {} bytes to file (frame {})", 
                       data_size, frames_recorded_ + 1);
  }
...
class App : public holoscan::Application {
 public:
  void compose() override {
   using namespace holoscan;
    ...
    auto adv_net_media_rx = make_operator<ops::AdvNetworkMediaOpRx>(
        "advanced_network_media_rx", from_config("advanced_network_media_rx"),
    ...
    if (from_config("media_player_config.visualize").as<bool>()) {
      const auto input_format = get_video_input_format(from_config(
          "media_player_config.input_format").as<std::string>());
      auto format_converter =
        make_operator<ops::orsi::FormatConverterOp>("format_converter",
                                                    Arg("in_dtype", input_format),
                                                    Arg("out_dtype", "rgba8888"),
      ...
      auto visualizer =
        make_operator<ops::HolovizOp>("visualizer",
                                      ...
                                      Arg("allocator") = allocator);
      add_flow(adv_net_media_rx, format_converter);
      add_flow(format_converter, visualizer, {{"", "receivers"}});
    } else if (from_config("media_player_config.write_to_file").as<bool>()) {
      auto frames_writer = make_operator<ops::FramesWriterOp>("frames_writer",
        from_config("frames_writer"));
 ...
 private:
  std::string get_video_input_format(const std::string& format) const {
    ...
    if (format_lower == "rgb888" || format_lower == "rgb") return std::string("rgb888");
    ...
    if (format_lower == "ycbcr-4:2:0" || format_lower == "yuv420" || format_lower == "yuv-420" ||
      ...
      return std::string("yuv420");

    if (format_lower == "nv12" || format_lower == "nv12_er" || format_lower == "nv12_709" ||
        format_lower == "nv12_709_er")
      return std::string("nv12");

    HOLOSCAN_LOG_INFO("Unsupported video sampling format: {}.", format);
    throw std::runtime_error("Unsupported video sampling format: " + format);
  }
