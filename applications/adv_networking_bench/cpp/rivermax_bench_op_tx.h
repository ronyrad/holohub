...
class AdvNetworkingBenchRivermaxTxOp : public Operator {
 public:
  ...
  void compute(InputContext&, OutputContext& op_output, ExecutionContext&) override {
    ...
    static int sent = 0;
    static int err = 0;
    // Set port_id in compute cause it is not set in setup due to the fact that
    // ANO Manager is initialized after setup is called.
    if (port_id_ < 0) {
      port_id_ = address_to_port(address_.get());
      if (port_id_ < 0) {
        HOLOSCAN_LOG_ERROR("Failed to get port id for address {}", address_.get());
        throw std::runtime_error("Failed to get port id for address");
      }
      HOLOSCAN_LOG_INFO("Port id for address {} is {}", address_.get(), port_id_);
    }
    if (!cur_msg_) {
    ...
    sent++;
    op_output.emit(cur_msg_, "burst_out");
    cur_msg_ = nullptr;
